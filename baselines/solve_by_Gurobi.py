# this file provides Gurobi tsp solver wrapper for our research
...
from solve_tsp_baselines import solve_tsp_instance_by_Gurobi
...
def create_tsp_baselines_by_Gurobi(args):
    ...
    for instance in tsp_instances:
        ...
        tour = solve_tsp_instance_by_Gurobi(dist_matrix)
        end_time = time.time()
    ...
    print(f"Gurobi Inference Finished!")
    print(f"[*] Summary           : Gurobi solver")
...
def create_baselines_by_Gurobi(args):
    if args.problem_type.lower() == "tsp":
        create_tsp_baselines_by_Gurobi(args)
...
def main():
    args = parse()
    create_baselines_by_Gurobi(args)
